...
**변경 파일**
- `app/services/image_processing.py` — `ImagePreprocessor` 클래스 + `get_image_preprocessor()` 팩토리
- `pyproject.toml` — `opencv-python-headless>=4.8.0`, `numpy>=1.26.0` 추가

**5계층 처리 순서**
| 계층 | 처리 | 목적 |
|---|---|---|
| 1 | 그레이스케일 변환 | 채널 단순화, 이후 처리 연산 최소화 |
| 2 | Gaussian blur (3×3) | 카메라 노이즈 / JPEG 아티팩트 제거 |
| 3 | CLAHE | 부분 그늘·과노출에도 텍스트 가독성 유지 |
| 4 | Deskew (회전 보정) | 최소 외접 사각형 각도 기반, 0.5° 미만 무시 |
| 5 | Dewarp (원근 변환) | 최대 사각형 윤곽 → 원근 보정, 조건 미충족 시 원본 유지 |

**설계 결정**
- 각 계층 독립 실패 허용: 한 계층 예외 발생 → 해당 계층 건너뛰고 다음 계층 진행
- 전체 실패 시 원본 이미지 그대로 반환 → OCR 파이프라인 중단 없음
- `opencv-python-headless` 선택: 서버(Docker) 환경에서 GUI 의존성 제거
- `get_image_preprocessor()` 팩토리: 추후 설정 주입 확장 대비

**TODO (프론트 팀 확인 후)**
- ML Kit이 전송 전 어느 단계까지 전처리하는지 확인
- 중복 처리 계층 제거 (예: ML Kit 그레이스케일 처리 시 1계층 스킵)
- Dewarp 임계값(현재 20%) 실제 촬영 환경에 맞게 조정

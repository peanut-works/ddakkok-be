...
조도 개선 계층은 제외됨: 실측상 상용 OCR(CLOVA) 정확도를 핵심 필드 이득 없이
hiconf -5~7% 떨어뜨려 파이프라인에서 뺐다. 구현은 _enhance_lighting에 보존
(약한/온디바이스 OCR용).
...
class ImagePreprocessor:
    ...
    # ------------------------------------------------------------------
    # (파이프라인 미포함) 조도 개선 — 약한/온디바이스 OCR용으로 보존
    # ------------------------------------------------------------------
